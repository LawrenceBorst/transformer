...
class FeedForward(torch.nn.Module):
    ...
    def __init__(self, model_dim: int, hidden_dim: int) -> None:
        ...
        self._w_1 = torch.nn.Parameter(
            torch.rand(model_dim, hidden_dim) * he_1, requires_grad=True
        )
        self._b_1 = torch.nn.Parameter(torch.zeros(hidden_dim), requires_grad=True)
        self._w_2 = torch.nn.Parameter(
            torch.rand(hidden_dim, model_dim) * he_2, requires_grad=True
        )
        self._b_2 = torch.nn.Parameter(torch.zeros(model_dim), requires_grad=True)
